...
try:
    ...
    matug = json.loads(
        subprocess.check_output(["matugen", "image", wallpaper_path, "--json", "hex"])
    )["colors"]
...
def move_to_group(window, direction) -> None:
    if window.qtile.current_group is None:
        return
...
keys = [
    ...
    Key(
        [mod, "control"],
        "Print",
        lazy.spawn(dir_path + "/print-screen.sh" + " selection"),
    ),
    ...
    Key(
        ...
        lazy.spawn(dir_path + "/volume-control.sh" + " raise"),
    ),
    Key(
        [],
        "XF86AudioLowerVolume",
        lazy.spawn(dir_path + "/volume-control.sh" + " lower"),
    ),
...
screens = [
    Screen(
        ...
        bottom=bar.Bar(
            [
                widget.CurrentLayout(mode="icon", background=primary_color),
                widget.GroupBox(
                ...
                widget.Clock(
                    background=primary_color, format="%H:%M", update_interval=60.0
                ),
            ],
            size=bar_size // 2,
